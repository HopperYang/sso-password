...
| 2 | 密码本体加密 | **RSA-OAEP-256 + AES-256-GCM 混合加密**（见 §1.1.1）：RSA 仅封装随机 AES-256 密钥与约定元数据；密码明文仅由 AES-256-GCM 保护。 |
...
- **混合加密（推荐表述）**：**RSA-OAEP-256 + AES-256-GCM**。其中 **RSA-OAEP-256** 即 **RSA-OAEP with SHA-256**（MGF1 与 OAEP 哈希均为 SHA-256；与 Web Crypto API、Java JCA 常见配置一致；业界常简称 RSA-OAEP-256）。
- **RSA 职责**：仅用于封装随机生成的 **AES-256** 对称密钥（及按协议约定的少量元数据）；**禁止**用 RSA 直接加密整段 PIN/密码明文（长度限制且非最佳实践）。
- **AES 职责**：**AES-256-GCM** 加密 UTF-8 密码明文，并提供完整性校验（auth tag）。
...
- SSO 使用**前端提交的临时 RSA 公钥**，按 **RSA-OAEP-256** 封装保护内层载荷的对称密钥，并以 **AES-256-GCM** 加密内层 JSON，将**外层信封**（`encryptedKey` / `iv` / `ciphertext` / `tag` 等）返回前端（可 Base64）；与 §1.1.1 算法定稿一致。
...
### 1.3 密码混合加密（RSA-OAEP-256 + AES-256-GCM）

- 前端生成随机 **AES-256** 密钥与 IV/nonce，使用 **AES-256-GCM** 加密 UTF-8 密码明文，得到密文与 auth tag。
- 使用服务端下发的会话 **RSA 公钥** 对「AES 密钥 +（可选）IV」做 **RSA-OAEP-256** 封装，得到 `encryptedKey`（若长度策略需要，可仅封装 AES 密钥，IV 可随密文明文传输由约定约束）。
- 提交接口：`sessionId`、`encryptedKey`、`ciphertext`、`iv`、`tag`（字段名可统一 snake_case 或 camelCase，前后端一致即可）。

### 1.4 服务端解密

- 校验 `sessionId` 有效且未过期；用会话私钥解 `encryptedKey` 得 AES 密钥；再 **AES-256-GCM** 解密得到密码明文（POC 可记录「成功/失败」，禁止日志打印明文密码）。
...
- **银行卡 PIN 子路由（`/bank/pin`，档位 A）**：仅 **6 位数字**；界面为 **6 个圆点** 表示进度（圆内小点表示已输入，**不展示数字**）；**第一屏输满 6 位后自动进入第二屏「请再次输入」**；两遍一致后提交；提供「返回上一步」。
...
- **两套 POC 的优缺点、威胁面与共用局限**：见根目录 [`README.md`](README.md)「优缺点与安全风险」与「安全说明」。
